...
// Your code goes here:
    ...
    template <int T, int ... V> // V = Vector
    struct Prepend<T, Vector<V...>>{
        using type = Vector<T, V...>;
    };
// ^ Your code goes here
...
// Your code goes here:
    template <int T, typename V>
    using PrependT = typename Prepend<T, V>::type;
// ^ Your code goes here
...
// Your code goes here:
    ...
    struct Append<T, Vector<V...>> {
        using type = Vector<V..., T>;
    };
...
// ^ Your code goes here
    ...
    template <int T, int ... V>
    struct PopBack<Vector<T, V...>> {
        using type = typename Prepend<T, typename PopBack<Vector<V...>>::type>::type;
    };
...
// the case when they are the same
    template <int T, int ... V>
    struct Rem<T, Vector<T, V...>> {
        using type =  Vector<V...>;
    };

// the case when they are different ? parse next

    template <int T, int U, int ...V>
    struct Rem<T, Vector<U, V...>> {
        using type = typename Prepend<T, typename Rem<T, Vector<V...>>::type>::type;
    };


    template <int T, int ... V>
    struct RemoveFirst<T, Vector<V...>>{
        // remove the first occurance of it
        // I need to recursively parse the V... and if
        using type = typename Rem<T, Vector<V...>>::type;
    };
// ^ Your code goes here
...
// ^ Your code goes here
    ...
    template <int T, int ... V>
    struct RemAll<T, Vector<V...>> {
        using type = Vector<>;
    };

// when they are the same
    template <int T, int ... V>
    struct RemAll<T, Vector<T, V...>> {
        using type = typename RemAll<T, Vector<V...>>::type;
    };
...
// when they are different
    ...
    template <int T, int U , int ... V>
    struct RemAll<T, Vector<U, V...>> {
        using type = typename Prepend<U, typename RemAll<T, Vector<V...>>::type>::type;
    };
...
// base case
    ...
    template <int T, int ... V>
    struct Length<Vector<T, V...>> {
        constexpr static int value = 1 + Length<Vector<V...>>::value;
    };
