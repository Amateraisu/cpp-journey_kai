...
static_assert(std::is_same_v<Vector<1,23,4>, Vector<1,23,4>>);
...
static_assert(std::is_same_v<Prepend<2, Vector<2,3>>::type, Vector<2,2,3>>);
...
static_assert(std::is_same_v< Append<5, Vector<1,2,3>>::type , Vector<1,2,3,5> >);
static_assert(std::is_same_v< Append<5, Vector<>>::type , Vector<5> >);
/**
 * 6. Define PopBack.
 */
...
struct PopBack<Vector<I, Is...>> {
  using type =  typename Prepend<I, typename PopBack<Vector<Is...>>::type>::type;
};

// ^ Your code goes here

static_assert(std::is_same_v< PopBack<Vector<1,2,3,4>>::type , Vector<1,2,3> >);
static_assert(std::is_same_v< PopBack<Vector<1,2,3,5>>::type , Vector<1,2,3> >);
static_assert(std::is_same_v< PopBack<Vector<1>>::type , Vector<>>);
static_assert(std::is_same_v< PopBack<Vector<8,7,9>>::type , Vector<8,7>>);
/**
...
static_assert(std::is_same_v<RemoveFirst<1, Vector<1,3,2>>::type, Vector<3,2>>);
static_assert(std::is_same_v<RemoveFirst<1, Vector<>>::type, Vector<>>);
static_assert(std::is_same_v<RemoveFirst<1, Vector<222,2,2,2>>::type, Vector<222,2,2,2>>);
/**
...
// ^ Your code goes here
static_assert(std::is_same_v<RemoveAll<8, Vector<1,9,2,9,3,9>>::type, Vector<1,9,2,9,3,9>>);
static_assert(std::is_same_v<RemoveAll<9, Vector<1,9,2,9,3,9>>::type, Vector<1,2,3>>);
static_assert(std::is_same_v<RemoveAll<7, Vector<>>::type, Vector<>>);
...
template <int... Is>
constexpr int Length<Vector<Is...>> = sizeof...(Is);
// ^ Your code goes here
...
struct Min<Vector<I, Is...>> {
  constexpr static int value = I < Min<Vector<Is...>>::value ? I : Min<Vector<Is...>>::value;
};
...
static_assert(Min<Vector<2,2,0>>::value == 0, "Minimum value should be 0");
static_assert(Min<Vector<3,1,2>>::value == 1, "Minimum value should be 1");
static_assert(Min<Vector<1,2,3>>::value == 1, "Minimum value should be 1");
static_assert(Min<Vector<3,2,1>>::value == 1, "Minimum value should be 1");
...
struct Sort<Vector<Is...>> {
  // sort the Is....
  // first I need to get the minimum out first
  constexpr static int mini = Min<Vector<Is...>>::value;
  using type2 = typename RemoveFirst<mini, Vector<Is...>>::type;
  // recursively add it into my current vector and then sort it
  using type = typename Prepend<mini, typename Sort<type2>::type>::type;
};
...
// template <int I, int U, int... Is>
// struct Uniq<Vector<I, U, Is...>> {
//   using type = typename Uniq<Vector<U, Is...>>::type;
// };
